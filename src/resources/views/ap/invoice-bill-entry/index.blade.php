    ...
    <script type="text/javascript">
        ...
        * */
        ...
        $(document).on('click', '.vendorSelect', function () {
            ...
            /*if (($('#ap_switch_pay_vendor_search').val()) == '{{--{{\App\Enums\YesNoFlag::YES}}--}}') { //Block this Pavel-28-08-22
        ...
        * */
        ...
        $("#ap_add_search_account").on("click", function () {
            ...
            let costCenterDpt = $('#department :selected').val(); //Add costCenterDpt Part :pavel-31-01-22
            ...
            $("#acc_department").val($("#department :selected").text());
        ...
        function setAddAccountInfo(d) {
            if ($.isEmptyObject(d.bankAccountInfo)) {
                ...
                if (nullEmptyUndefinedChecked(d.bankAccountInfo.cost_center_dept_name)) {
                    ...
                    $("#department_cost_center").html('<option value="' + d.bankAccountInfo.cost_center_dept_id + '">' + d.bankAccountInfo.cost_center_dept_name + '</option>');
                }
        ...
        function enableDisableSaveBtn() {
            ...
            }*///alert($("#ap_distribution_flag").val())
...
*/
        ...
        $("#ap_search_account").on("click", function () {
            ...
            if (!nullEmptyUndefinedChecked(accId)) {
                 ...
                 * **/
            //Add IF Part :pavel-31-01-22
            else if (nullEmptyUndefinedChecked(costCenterDpt)) {
                $("#department").focus();
                $('html, body').animate({scrollTop: ($("#department").offset().top - 400)}, 2000);
                $("#department").notify("Select Department First.", {position: 'left'});
        ...
        /*function getMaxCounter(tblObject){
            //This will need to maintain after fetching data from saved draft
            let orders = [];
            $(tblObject + " >tbody >tr").find(".lineCounter").each(function (i, o) {
                let order = $(this).val();
                orders.push(order);
            });
            ...
            if (!nullEmptyUndefinedChecked($("#ap_invoice_type").find(':selected').val())) {
                ...
                if (fieldsAreSet(['#ap_amount_ccy', '#ap_account_id', '#ap_account_name', '#ap_amount_lcy'])) {
                    if ($(selector).attr('data-type') == 'A') {
                        ...
                        //let maxOrder = getMaxCounter("#ap_account_table");
                        let count = Math.floor(Math.random() * 999);//maxOrder+1;
                        let partyId = "";
                        let partyName = "";
                        let partySubLedger = "";
                        let category = "";
                        ...
                        let html = '<tr>\n' +
                            ...
                            '<input type="hidden" class="lineCounter" name="lineCounter" value="'+count+'"/>'+
            ...
            * */
            ...
            function listBillRegister() {
                $('#bill_section').change(function (e) {
                    ...
                    /**  Pavel-Added this condition as per CR-0004213 **/
                    if ( (billSectionId == '{{\App\Enums\Ap\LBillSection::SALARY_SECTION_1}}') || (billSectionId == '{{\App\Enums\Ap\LBillSection::SALARY_SECTION_2}}') ) {
                        $("#emp_type_id").val("").prop('disabled', false);

                    } else {
                        $("#emp_type_id").val("").prop('disabled', true);
                    }
            ...
            //Added this when "$("#invoice_bill_entry_form").on("submit", function" not working
           /* $("#invoice_bill_entry_form_submit_btn").on("click", function (e) {
                e.preventDefault();
                Swal.fire({
                    title: "Are you sure?",
                    html: 'Submit' + '<br>' +
            ...
            $("#preview_btn").on("click", function (e) {
                ...
                //if ((invoiceArray[4] == '0') && nullEmptyUndefinedChecked(budgetHeadId) && (invoiceArray[5] == 'Y') && ($("#ap_without_budget_info").prop("checked") == false)) {
                if ($("#ap_without_budget_info").prop("checked") == false && $("#total_debit").val() == null) {
                    $("#b_head_id").focus();
                    $('html, body').animate({scrollTop: ($("#b_head_id").offset().top - 200)}, 2000);
                    $("#b_head_id").notify("Please Add Budget Booking Info.", {position: 'bottom'});
        ...
        function updateLineValuebzt(line) {
            ...
            $("#budget_amt" + line).val(transaction.b_utilized_amt);
            ...
            $("#b_amt" + line).val(transaction.b_amt);
            $("#u_amt" + line).val(transaction.b_utilized_amt);
            $("#b_available_amt" + line).val(transaction.b_available_amt);
            ...
            if (fieldsAreSet(['#b_head_id', '#b_head_name', '#budget_amt'])) {
                if ($(selector).attr('data-type') == 'A') {
                    ...
                    let html = '<tr>\n' +
                        ...
                        '<input tabindex="-1" type="hidden" name="addLineBudget[' + count + '][budget_department_id]" id="budget_department_id' + count + '" value="' + transaction.budget_department_id + '"/>' +
                        ...
                        '<input tabindex="-1" type="hidden" name="addLineBudget[' + count + '][b_amt]" id="b_amt' + count + '" value="' + transaction.b_amt + '"/>' +
                        '<input tabindex="-1" type="hidden" name="addLineBudget[' + count + '][u_amt]" id="u_amt' + count + '" value="' + transaction.u_amt + '"/>' +
                        '<input tabindex="-1" type="hidden" name="addLineBudget[' + count + '][b_available_amt]" id="b_available_amt' + count + '" value="' + transaction.b_available_amt + '"/>' +
                        '</td>\n' +
                        ...
                        '<td style="padding: 2px"><input tabindex="-1" name="addLineBudget[' + count + '][budget_amt]" id="budget_amt' + count + '" class="form-control form-control-sm debit" value="' + transaction.b_utilized_amt + '" readonly/></td></td>\n' +
                        '      <td style="padding: 2px;"><span style="text-decoration: underline" id="addLineBudget' + count + '" class="cursor-pointer danger editAccountBtnbzt" onclick="editAccountBudjet(this,' + count + ')" >Edit</span>|<span id="remove_btn_bzt' + count + '" onclick="removeLineRowBudjet(this,' + count + ')"><i class="bx bx-trash cursor-pointer"></i></span></td>\n' +
                        '  </tr>';
                ...
                resetField(['#b_head_id', '#b_amt', '#b_available_amt', '#b_utilized_amt', '#b_head_name', '#b_sub_category', '#b_category', '#b_type', '#budget_amt']);
        ...
        editAccountBudjet = function (selector, line) {
            ...
            $("#b_amt").val($("#b_amt" + line).val());
            $("#b_utilized_amt").val($("#u_amt" + line).val());
            $("#b_available_amt").val($("#b_available_amt" + line).val());
        ...
        $("#ap_without_budget_info").on("click", function () {
            ...
            if ($(this).prop('checked')) {
                $("#budget_department").removeAttr('required');
                enableDisableSaveBtn();
                ...
                enableDisableSaveBtn();
                $("#budget_department").prop('required', true);
