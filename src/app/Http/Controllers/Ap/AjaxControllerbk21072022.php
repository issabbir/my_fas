...
class AjaxControllerbk21072022 extends Controller
{
    ...
    public function invoiceReferenceList(Request $request)
    {
        ...
        if ($vendorId) {
            ...
            $invRefList = DB::select("select CPAACC.fas_ap_trans.get_ap_bill_payment_ref_mk (:p_gl_subsidiary_id, :p_vendor_id) from dual", ['p_gl_subsidiary_id' => $glSubsidiaryId, 'p_vendor_id' => $vendorId]);
        }
    ...
    {
        $fromPeriods = DB::select('select CPAACC.FAS_REPORT_CONTROL.get_posting_periods(:p_fiscal_year_id,:p_sort_order) from dual', ['p_fiscal_year_id' => $request->get("fiscal_year_id"), 'p_sort_order' => 'ASC']);
        $fromOptions = '';
        ...
        $toPeriods = DB::select('select CPAACC.FAS_REPORT_CONTROL.get_posting_periods(:p_fiscal_year_id,:p_sort_order) from dual', ['p_fiscal_year_id' => $request->get("fiscal_year_id"), 'p_sort_order' => 'DESC']);
        $toOptions = '';
