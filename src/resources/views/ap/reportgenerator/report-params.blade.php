    ...
    @if($report)
        ...
        @if($report->params)
            @foreach($report->params as $reportParam)
                ...
                @elseif($reportParam->component == 'specific_bill_register')
                    <div class="col-md-3">
                        <label for="p_bill_reg_id" class="@if($reportParam->requied_yn==\App\Enums\YesNoFlag::YES) required @endif">Bill Register</label>
                        <select @if($reportParam->requied_yn==\App\Enums\YesNoFlag::YES) required
                                @endif name="p_bill_reg_id" class="form-control " id="p_bill_reg_id">
                            <option value="">Select a bill</option>
                            @foreach(\App\Enums\Ap\ReportAPRegisters::AP_REGISTERS as $key=>$value)
                                <option value="{{$key}}">{{ $value}}</option>
                            @endforeach
                        </select>
                    </div>
                @elseif($reportParam->component == 'salary_head')
                         <div class="col-md-3">
                             <label for="{{$reportParam->param_name}}" class="@if($reportParam->requied_yn==\App\Enums\YesNoFlag::YES) required @endif">Salary Head</label>
                             <select @if($reportParam->requied_yn==\App\Enums\YesNoFlag::YES) required
                                                                     @endif name="{{$reportParam->param_name}}" class="form-control select2" id="{{$reportParam->param_name}}">
                                     <option value="">Select a head</option>
                                     @foreach($salaryHeads as $value)
                                             <option value="{{$value->salary_head_id}}">{{ $value->salary_head}}</option>
                                         @endforeach
                             </select>
                         </div>
...
{{--                            <option value="">Select Posting Period</option>--}}
                ...
                @elseif($reportParam->component == 'employee_type_id')
                    <div class="col-md-3">
                        <label for="{{$reportParam->param_name}}"
                               class="@if($reportParam->requied_yn==\App\Enums\YesNoFlag::YES) required @endif">{{$reportParam->param_label}}</label>
                        <select name="{{$reportParam->param_name}}" id="{{$reportParam->param_name}}"
                                ...
                                <option value="1,2">Officer (AD) & (OP) </option>
                                <option value="3,4">Staff (AD) & (OP) </option>
                                {{--@endforeach--}}
                        </select>
                    </div>
                @elseif($reportParam->component == 'tax_zone_list')
                    <div class="col-md-3">
                        ...
                        <select name="{{$reportParam->param_name}}" id="{{$reportParam->param_name}}" class="form-control select2" @if($reportParam->requied_yn==\App\Enums\YesNoFlag::YES) required @endif >
                            <option>Select Tax Zone</option>
                            {{--@foreach($users as $user)
                                <option value="{{$user->user_id}}">{{$user->emp_name}}({{$user->user_name}})</option>
                            @endforeach--}}
                            @foreach(\App\Enums\Common\LTaxZone::TAX_ZONE as $key=>$value)
                                <option  value="{{$key}}" > {{ $value}} </option>
                            @endforeach
                        </select>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        reportGetPostingPeriods($("#p_fiscal_year_id :selected").val(), APP_URL + '/account-payable/gl-posting-periods/'+$('#p_posting_period_id').data('nullable'), setPeriods);
        ...
        $("#p_fiscal_year_id").on('change', function () {
            ...
            if ($("#p_posting_period_id").length > 0) {
                ...
                destroyPostingCalenders();
                postingDateCalendarClickCounter = 0;
                reportGetPostingPeriods($(this).val(), APP_URL + '/account-payable/gl-posting-periods/'+$('#p_posting_period_id').data('nullable'), setPeriods);
            } else {
                resetPostingDateRelatedFields();
                destroyPostingCalenders();
                postingDateFromCalendarClickCounter = 0;
