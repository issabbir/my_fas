...
class InvoiceSalaryBillEntryController extends Controller
{
...
//       dd($data['subsidiary_type']);
        ...
        $lastPostingBatch = $this->lookupManager->findLastPostingBatchId(LGlInteModules::ACC_PAY_VENDOR, ApFunType::AP_INVOICE_BILL_ENTRY, $user_id);
         $lFasEmployeeType = LFasEmployeeType::all();
        $isRequired = [
        ...
        return view('ap.invoice-salary-bill-entry.index', compact('isRequired', 'customerCategory', 'fiscalYear', 'vendors', 'paymentMethod', 'paymentTerms', 'department', 'billSecs', 'data', 'coaParams', 'vendorType', 'vendorCategory', 'lastPostingBatch', 'lFasEmployeeType'));
    }
    ...
    public function insert(Request $request, $id = null)
    {
        ...
        try {
            ...
            $invoice_allowance = $request->post('invoice_allowance');
            $invoice_deduction = $request->post('invoice_deduction');

            if ($request->net_payable_amount == 0){
                $total_net_payable_amt = $request->total_pay_allowance;
            }else{
                $total_net_payable_amt =  $request->net_payable_amount;
            }
            ...
            $params = [
                ...
                'p_po_date' => ($request->post('ap_purchase_order_date') != null) ? HelperClass::dateFormatForDB($request->post('ap_purchase_order_date')) : '',
                ...
                'p_currency_code' => ($request->post('ap_payment_currency') !="")?$request->post('ap_payment_currency') :'BDT',
                'p_exchange_rate' => ($request->post('ap_exchange_rate') != "")? $request->post('ap_exchange_rate'): 1,
                'p_invoice_amount_ccy' => ($request->post('total_pay_allowance') != "") ? $request->post('total_pay_allowance') : 0, //11
                'p_invoice_amount_lcy' => ($request->post('total_pay_allowance') != "") ? $request->post('total_pay_allowance') : 0, //11
...
//                'p_payable_amount_ccy' => ($request->post('net_payable_amount') != "") ? $request->post('net_payable_amount') : 0,
//                'p_payable_amount_lcy' => ($request->post('net_payable_amount') != "") ? $request->post('net_payable_amount') : 0,
                'p_payable_amount_ccy' => $total_net_payable_amt ? $total_net_payable_amt : 0,
                'p_payable_amount_lcy' => $total_net_payable_amt ? $total_net_payable_amt : 0,
                'p_payment_terms_id' => ($request->post('ap_payment_terms') !="")?$request->post('ap_payment_terms'):1,
                'p_payment_due_date' => HelperClass::dateFormatForDB($request->post('ap_payment_due_date')),
                'p_payment_methods_id' => LApPayMethods::CHEQUE,
                ...
                'p_without_budget_yn' =>  YesNoFlag::YES,
                'p_system_generated_yn' => YesNoFlag::NO,
                'p_employee_type_id' => ($request->post('emp_type_id') != null) ? $request->post('emp_type_id') : '',
                'p_user_id' => auth()->id(),

                'o_invoice_id' => [
                    'value' => &$salary_invoice_id,
            ...
            DB::executeProcedure('CPAACC.fas_ap_trans.trans_ap_invoice_entry', $params);
            //Log::info($params);
            ...
            if ($params['o_status_code'] != "1") {
                ...
                $indv_allowance_amount = array_filter($invoice_allowance, function ($var) {
                    return ($var['pay-allowance-amt'] != null);
                });


                if ($request->post('total_pay_allowance') != '') {
                    foreach ($indv_allowance_amount as $key => $line) {
                        ...
                        ${'params' . $key} = [
                            ...
                            'p_invoice_id' => $salary_invoice_id,
                            ...
                            'p_document_ref' => $request->post('document_reference'),
                            'p_narration' => $request->post('narration'),
                            'p_department_id' => (integer)$request->post('department'),
                            'p_bill_sec_id' => (integer)$request->post('bill_section'),
                            'p_bill_reg_id' => (integer)$request->post('bill_register'),
                            'p_gl_acc_id' => (integer)$line['gl_acc_id'],
                            'p_gl_subsidiary_id' => null, // ($request['ap_party_sub_ledger'] != null) ? $request['ap_party_sub_ledger'] : null,  //Add Pavel-07-07-22
                            'p_vendor_id' => null , // ($request['ap_vendor_id'] != null) ? $request['ap_vendor_id'] : null,   //Add Pavel-07-07-22
                            'p_currency_code' => Currencies::O_BD,
                            'p_exchange_rate' => 1,
                            'p_amount_ccy' => $line['pay-allowance-amt'],
                            'p_amount_lcy' => $line['pay-allowance-amt'],
                            'p_additional_acc_yn' => null,
                        ...
                        try {
                            DB::executeProcedure('CPAACC.fas_ap_trans.trans_ap_invoice_lines', ${'params' . $key});
                            //Log::info(${'params' . $key});
                        } catch (\Exception $e) {

                            DB::rollBack();
                            return response()->json(["response_code" => ${"status_code" . $key}, "response_msg" => ${"status_message" . $key} . 'Add Acc']);
                            //Log::info("Internal Error occurred during AP line entry.");
                        }
                ...
                $invoice_deduction_amount = array_filter($invoice_deduction, function ($var) {
                    return ($var['pay_amt_deduction'] != null);
                });

                $lines = $invoice_deduction_amount;

                if ($request->post('total_pay_deduction') != '') {
                    foreach ($lines as $key => $line) {
                        ...
                        ${'params' . $key} = [
                            ...
                            'p_module_id' => LGlInteModules::ACC_PAY_VENDOR,    'p_function_id' => ApFunType::AP_INVOICE_BILL_ENTRY,
                            'p_trans_period_id' => (integer)$request->post('period'),
                            'p_trans_date' => HelperClass::dateFormatForDB($request->post('posting_date')),
                            'p_document_date' => HelperClass::dateFormatForDB($request->post('document_date')),
                            'p_document_no' => $request->post('document_number'),
                            ...
                            'p_currency_code' => Currencies::O_BD,
                            'p_exchange_rate' => 1,
                            'p_amount_ccy' => $line['pay_amt_deduction'],
                            'p_amount_lcy' => $line['pay_amt_deduction'],
                            'p_additional_acc_yn' => YesNoFlag::YES,
                            'p_user_id' => auth()->id(),
                            'o_status_code' => &${'status_code' . $key},
                            'o_status_message' => &${'status_message' . $key}
                        ];
                        ...
                        if (${'status_code' . $key} != "1") {
                            ...
                            //throw new \Exception(${"status_message" . $key}, ${'params' . $key});
                            return response()->json(["response_code" => ${"status_code" . $key}, "response_msg" => ${"status_message" . $key} . 'Add Acc']);
                        }
                    }
                ...
                //dd($request,is_null($request->file('attachment')), $request->file());
                if (!is_null($request->file('attachment'))) {
                    ...
                    foreach ($files as $key => $file) {
                        if ($file['file'] && $descriptions[$key]['actionType'] == "I") {
                            ...
                            DB::executeProcedure('CPAACC.fas_ap_trans.trans_ap_invoice_docs', ${"file_params" . $key});
                            //Log::info(${'file_params' . $key});
                ...
                if ($salary_invoice_id) {
                    ...
                    $wkMappingParams = [
                        ...
                        'P_REFERANCE_ID' => $salary_invoice_id,
                        'P_TRANS_PERIOD_ID' => $request->post('period'),
                        'P_INSERT_BY' => auth()->id(),
    ...
    public function preview(Request $request)
    {
        ...
        $payable_amount = $request->post('total_pay_allowance') - $request->post('total_pay_deduction');


        try {
            $responses = DB::select('select * from table (cpaacc.fas_ap_trans.get_ap_invoice_preview(
                ...
                [
                    ...
                    , 'p_payable_amount_lcy' => (double)$payable_amount
                ]);
            ...
            $lFasEmployeeType = LFasEmployeeType::select('employee_type_name')->where('employee_type_id', $request->post('emp_type_id'))->first();
            ...
            $data = [
                ...
                , 'budget' => $content//isset($head) ? $head->budget_head_name : ''
                , 'narration' => $request->post('narration')
                , 'invoice_allowance' => $invoice_allowance
                , 'lFasEmployeeType' => $lFasEmployeeType->employee_type_name
            ];

            return response()->json(['response_code' => 1, 'response_msg' => 'success', 'table_content' => view('ap.ap-common.salary_dr_cr', compact('responses', 'indv_allowance_amount','invoice_deduction_amount', 'data'))->render()]);
        } catch (\Exception $e) {
    ...
    {
        if (isset($id)) {
            $invRefList = DB::select('select fas_ap_trans.get_pmis_allowance_heads(:emp_type_id) from dual', ['emp_type_id' => $id]);

        }

        $html = view('ap.ajax.pay-allowance-list')->with(compact('invRefList'))->render();
        $jsonArray = [
            'html' => $html
        ];
    ...
    {
        if (isset($id)) {
            $deductionList = DB::select('select fas_ap_trans.get_pmis_deduction_heads (:emp_type_id) from dual', ['emp_type_id' => $id]);
        }

        $html = view('ap.ajax.pay-deduction-list')->with(compact('deductionList'))->render();
