...
class InvoiceBillEntryController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('ap.invoice-bill-entry.index', compact('isRequired','customerCategory', 'fiscalYear', 'vendors', 'paymentMethod', 'paymentTerms', 'department', 'billSecs', 'data', 'coaParams', 'vendorType', 'vendorCategory', 'lastPostingBatch', 'lFasEmployeeType'));
    }
    ...
    {//dd($request->all());
        ...
        try {
            ...
            $params = [
                ...
                'p_without_budget_yn' => ($request->post('ap_without_budget_info') != "") ? $request->post('ap_without_budget_info') : YesNoFlag::NO,
                ...
                if ($request->post('ap_distribution_flag') == '0') {
                    foreach ($lines as $key => $line) {
                        ...
                        ${'params' . $key} = [
                            ...
                            'p_narration' => $request->post('narration'),
                            'p_department_id' => (integer)$request->post('department'),
                            //'P_budget_dept_id' => (integer)$request->post('budget_department'),            /********Added on: 06/06/2022, sujon -remove-pavel-07-06-22**********/
                            'p_bill_sec_id' => (integer)$request->post('bill_section'),
                            'p_bill_reg_id' => (integer)$request->post('bill_register'),
                        ...
                        if (${'status_code' . $key} != "1") {
                            DB::rollBack();
                            //throw new \Exception(${"status_message" . $key}, ${'params' . $key});
                            return response()->json(["response_code" => ${"status_code" . $key}, "response_msg" => ${"status_message" . $key} . "Distribution"]);
                        }
                    }
                }

                $addLineBudget = $request->get('addLineBudget');//dd($addLineBudget);
                if($addLineBudget){
                    $bud_status_code = sprintf("%4000d", "");
                    $bud_status_message = sprintf("%4000s", "");
                    foreach ($addLineBudget as $key => $line) {
                        $budgetParams = [
                            'p_invoice_id' => $invoice_id,
                            'p_budget_dept_id' => $line['budget_department_id'],
                            'p_budget_head_id' => $line['b_head_id'],
                            'p_budget_utilize_amt' => $line['budget_amt'],
                            'p_user_id' => auth()->id(),
                            'o_status_code' => &$bud_status_code,
                            'o_status_message' => &$bud_status_message
                        ];
                        //dd($budgetParams);
                        //Log::info($budgetParams);
                        DB::executeProcedure('CPAACC.fas_ap_trans.trans_ap_invoice_budget', $budgetParams);
                        //Log::info($budgetParams);
                        //dd($budgetParams);
                    }
                }
...
//dd($request->all());
        ...
        try {
            ...
            if($addLineBudget){
                foreach ($addLineBudget as $key => $line) {
                    $head = FasBudgetHead::select('budget_head_name', 'budget_head_id')->where('BUDGET_HEAD_ID',$line['b_head_id'])->first();
                    $content .=$head->budget_head_id.'-'.$head->budget_head_name.'; ';
                }
            }//dd($content);
