...
use App\Enums\Ap\LBillSection;
use App\Enums\Common\LGlInteModules;
use App\Enums\Common\LTaxZone;
...
class ReportGeneratorController extends Controller
{
    ...
    {
        $module = ModuleInfo::AP_MODULE_ID;
        ...
        return view('ap.reportgenerator.index', compact('reports'));
    }
    ...
    public function reportParams($id)
    {
        ...
        $modules = $this->lookupManager->getAPModuleList();
        ...
        $oldFiscalYear = $this->lookupManager->getOldFiscalYear();

        $billSecs = $this->lookupManager->findLBillSec();
        $billRegister = LBillRegister::where('bill_sec_id', '=', LBillSection::CASH_SECTION)->get();
        $apFuncType = $this->functions->where("module_id", "=", LGlInteModules::ACC_PAY_VENDOR)->where("function_parent_id", "!=", null)->orderBy("function_id", "ASC")->get();
        ...
        $salaryHeads = DB::select("select fas_report_control.get_pmis_salary_head from dual");
        $lFasEmployeeType = LFasEmployeeType::all();
        //Write your dependency elements query here
        return view('ap.reportgenerator.report-params', compact('salaryHeads','users', 'fiscalYear', 'modules', 'oldFiscalYear', 'apFuncType', 'vendors', 'subsidiary_type', 'closingEvents', 'department', 'billSecs','billRegister', 'lFasEmployeeType','report'))->render();
    }
}
