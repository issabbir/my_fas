...
class InvoiceBillAuthorizeController extends Controller
{
    ...
    public function searchInvoiceBillAuthorize(Request $request)
    {
        ...
        $customer = $request->post('ap_customer');
        ...
        $data = DB::select("select cpaacc.fas_ar_trans.get_ar_invoice_auth_list (
            :p_fiscal_year_id,
            :p_trans_period_id,
            :p_bill_sec_id,
            :p_bill_reg_id,
        ...
        return datatables()->of($data)
            ...
            ->editColumn('action', function ($data) use ($filteredData) {
                ...
                if ($data->workflow_approval_status == ApprovalStatus::PENDING) {
                    if (HelperClass::findUserPermissionKey(ModuleInfo::AR_MODULE_ID, RolePermissionsKey::CAN_QUICK_AUTHORIZE_AR_INVOICE_MAKE)) {
                ...
                return $approveBtn . '<a href="' . route('ar-invoice-bill-authorize.approval-view', [$data->invoice_id, 'wk_map_id' => $data->workflow_mapping_id, 'user_id' => $data->login_user_id, 'wk_ref_status' => $data->workflow_reference_status, 'filter' => $filteredData]) . '">
    ...
    public function approveRejectCancel(Request $request, $wkMapId = null, $filter = null)
    {
        ...
        if ($request->get('approve_reject_value') == ApprovalStatus::CANCEL) {
            try {
                ...
                DB::executeProcedure('CPAACC.fas_ar_trans.trans_ar_invoice_cancel', $params);
        ...
        if ($response['o_status_code'] != 1) {
            ...
            return redirect()->back()->with('message', 'error|' . $response['o_status_message'])->withInput();
        }

        /*session()->flash('m-class', 'alert-success');
        session()->flash('message', $message);

        return redirect()->route('ar-invoice-bill-authorize.index');*/
        session()->flash('m-class', 'alert-success');
        session()->flash('message', $message);
